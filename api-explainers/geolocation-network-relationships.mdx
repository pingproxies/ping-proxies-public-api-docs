...
# Object Relationships in the Ping Proxies API
...
<img src="/images/object-hierarchy.svg" alt="Ping Proxies Object Hierarchy" />

The diagram above illustrates the hierarchical relationships between different objects in the Ping Proxies API. Let's explore these relationships in detail:
...
{
  "data": [
    "socks5h://1_oGDQZ_c_gb_city_london_s_DDINPY7TQ0781XEO:vxqaFAA99F@residential.pingproxies.com:8000",
    "socks5h://1_oGDQZ_c_gb_city_london_s_XIINPY7TQ0781XEA:vxqaFAA99F@residential.pingproxies.com:8000",
    "socks5h://1_oGDQZ_c_gb_city_london_s_PPINPY7TQ0781XEB:vxqaFAA99F@residential.pingproxies.com:8000",
    "socks5h://1_oGDQZ_c_gb_city_london_s_QQINPY7TQ0781XEC:vxqaFAA99F@residential.pingproxies.com:8000",
    "socks5h://1_oGDQZ_c_gb_city_london_s_RRINPY7TQ0781XED:vxqaFAA99F@residential.pingproxies.com:8000"
  ],
...
### Combined Geographic and Network Filtering

You can combine both geographic and network attributes for highly specific targeting:

```bash
# Find all ISP proxies in Texas on AT&T's network
curl --request GET \
  --url 'https://api.pingproxies.com/1.0/public/user/proxy/search?proxy_type=isp&subdivision_id=us-tx&asn_id=7018' \
  --header 'X-API-Public-Key: your_public_key' \
  --header 'X-API-Private-Key: your_private_key'
```

## Service Fulfillment Filters and Object Relationships

When purchasing proxies through the checkout process, you can use service fulfillment filters to target specific geographic or network attributes:
...
```json
{
  ...
  "service_fulfillment_filter": {
    "asn_id": 7018,        // AT&T
    "subdivision_id": "us-tx",  // Texas
    "city_id": 75202       // Dallas
  }
}
```
