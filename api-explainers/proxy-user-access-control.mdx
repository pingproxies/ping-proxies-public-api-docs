...
**Step 3:** Add more services as needed
...
**Scenario:** Your company has SEO, Marketing, and Research teams. Each needs access to different proxy pools.

**Solution:** Create service-restricted proxy users for each team:

- `seo_team` → Access to `API-SEO-POOL-001`
- `marketing_team` → Access to `API-MARKETING-001`
- `research_team` → Access to `API-RESEARCH-001`

### Proxy Reselling

**Scenario:** You resell individual proxies to customers and need to ensure each customer can only access their purchased proxies.

**Solution:** Create proxy-restricted proxy users:

- `customer_001` → Access to proxies: `proxy_a`, `proxy_b`, `proxy_c`
- `customer_002` → Access to proxies: `proxy_d`, `proxy_e`

### Multi-Environment Setup

**Scenario:** Development team needs access to all proxies, but production systems should only use specific high-quality services.

**Solution:**
- Development: `dev_user` with `access_type: "all"`
- Production: `prod_user` with `access_type: "service_restricted"` → `API-PREMIUM-001`
