...
# Exporting Static Proxies to a List Using the List Endpoint
...
def get_static_proxy_list(format_type="standard", protocol="http"):
    """
    ...
    params = {
        ...
        "list_protocol": protocol,   # Protocol to use (http or socks5)
...
def export_to_file(proxy_list, format_type="standard", filename=None):
    """Export proxy list to a file."""
    if not filename:
        # Create a default filename with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"proxies_{format_type}_{timestamp}.txt"
...
if __name__ == "__main__":
    # Get HTTP proxies in different formats
    standard_list = get_static_proxy_list("standard", "http")
    http_list = get_static_proxy_list("http", "http")
    
    # Get SOCKS proxies in different formats
    socks5_list = get_static_proxy_list("socks5", "socks5")
    socks5h_list = get_static_proxy_list("socks5h", "socks5")
    
    # Export to files
    if standard_list:
        export_to_file(standard_list, "standard")
    if http_list:
        export_to_file(http_list, "http")
    if socks5_list:
        export_to_file(socks5_list, "socks5")
    if socks5h_list:
        export_to_file(socks5h_list, "socks5h")
    ...
    if socks5h_list:
        print(f"SOCKS5H format: {socks5h_list[0]}")
```
...
/**
 * Retrieve all static proxies using the list_by_search endpoint
 * @param {string} formatType - Format of the returned proxies (standard, http, socks5, socks5h)
 * @param {string} protocol - Protocol to use (http or socks5)
 * @returns {Promise<Array>} List of formatted proxy strings
 */
async function getStaticProxyList(formatType = 'standard', protocol = 'http') {
  // Set up the parameters for list_by_search
 ...
 */
function exportToFile(proxyList, formatType = 'standard') {
  // In browser context, create a download
  if (typeof window !== 'undefined') {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `proxies_${formatType}_${timestamp}.txt`;
...
async function main() {
  // Get HTTP proxies in different formats
  const standardList = await getStaticProxyList('standard', 'http');
  const httpList = await getStaticProxyList('http', 'http');
  
  // Get SOCKS proxies in different formats
  const socks5List = await getStaticProxyList('socks5', 'socks5');
  const socks5hList = await getStaticProxyList('socks5h', 'socks5');
  
  // Export to files
  if (standardList.length > 0) {
    exportToFile(standardList, 'standard');
  }
  if (httpList.length > 0) {
    exportToFile(httpList, 'http');
  }
  if (socks5List.length > 0) {
    exportToFile(socks5List, 'socks5');
  }
  if (socks5hList.length > 0) {
    exportToFile(socks5hList, 'socks5h');
  }
  ...
  if (socks5hList.length > 0) {
    console.log(`SOCKS5H format: ${socks5hList[0]}`);
  }
}
 ...
 */
function getStaticProxyList($apiPublicKey, $apiPrivateKey, $baseUrl, $formatType = 'standard', $protocol = 'http') {
    // Headers for authentication
    $headers = [
...
function exportToFile($proxyList, $formatType = 'standard') {
    ...
    $filename = "proxies_{$formatType}_{$timestamp}.txt";
...
// Example usage
// Get HTTP proxies in different formats
$standardList = getStaticProxyList($apiPublicKey, $apiPrivateKey, $baseUrl, 'standard', 'http');
$httpList = getStaticProxyList($apiPublicKey, $apiPrivateKey, $baseUrl, 'http', 'http');

// Get SOCKS proxies in different formats
$socks5List = getStaticProxyList($apiPublicKey, $apiPrivateKey, $baseUrl, 'socks5', 'socks5');
$socks5hList = getStaticProxyList($apiPublicKey, $apiPrivateKey, $baseUrl, 'socks5h', 'socks5');
...
if (!empty($socks5hList)) {
    echo "SOCKS5H format: " . $socks5hList[0] . "\n";
}
?>
```
...
func GetStaticProxyList(formatType string, protocol string) ([]string, error) {
	...
	q.Add("list_protocol", protocol)     // Protocol to use (http or socks5)
...
func ExportToFile(proxyList []string, formatType string) (string, error) {
	// Create a default filename with timestamp
	timestamp := time.Now().Format("20060102_150405")
	filename := fmt.Sprintf("proxies_%s_%s.txt", formatType, timestamp)
...
func main() {
	// Get HTTP proxies in different formats
	standardList, err := GetStaticProxyList("standard", "http")
	if err != nil {
		fmt.Printf("Error retrieving standard proxies: %v\n", err)
	}
	
	httpList, err := GetStaticProxyList("http", "http")
	if err != nil {
		fmt.Printf("Error retrieving HTTP proxies: %v\n", err)
	}
	
	// Get SOCKS proxies in different formats
	socks5List, err := GetStaticProxyList("socks5", "socks5")
	if err != nil {
		fmt.Printf("Error retrieving SOCKS5 proxies: %v\n", err)
	}
	
	socks5hList, err := GetStaticProxyList("socks5h", "socks5")
	if err != nil {
		fmt.Printf("Error retrieving SOCKS5H proxies: %v\n", err)
	}
	
	// Export to files
	if len(standardList) > 0 {
		_, err := ExportToFile(standardList, "standard")
		if err != nil {
	...
	if len(httpList) > 0 {
		_, err := ExportToFile(httpList, "http")
		if err != nil {
	...
	if len(socks5List) > 0 {
		_, err := ExportToFile(socks5List, "socks5")
		if err != nil {
			fmt.Printf("Error exporting SOCKS5 proxies: %v\n", err)
		}
	}
	
	if len(socks5hList) > 0 {
		_, err := ExportToFile(socks5hList, "socks5h")
		if err != nil {
			fmt.Printf("Error exporting SOCKS5H proxies: %v\n", err)
		}
	}
...
public class ExportStaticProxies {
    ...
    /**
     * Retrieve all static proxies using the list_by_search endpoint
     * @param formatType Format of the returned proxies (standard, http, socks5, socks5h)
     * @param protocol Protocol to use (http or socks5)
     * @return List of formatted proxy strings
     */
    public static List<String> getStaticProxyList(String formatType, String protocol) 
            throws IOException, InterruptedException, URISyntaxException {
        
        // Build the URI with query parameters
        URI uri = new URI(String.format("%s/user/proxy/list_by_search", BASE_URL))
                .resolve("?list_format=" + formatType
     ...
     */
    public static String exportToFile(List<String> proxyList, String formatType) throws IOException {
        // Create a default filename with timestamp
        LocalDateTime now = LocalDateTime.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss");
        String timestamp = now.format(formatter);
        String filename = String.format("proxies_%s_%s.txt", formatType, timestamp);
    ...
    public static void main(String[] args) {
        try {
            // Get HTTP proxies in different formats
            List<String> standardList = getStaticProxyList("standard", "http");
            List<String> httpList = getStaticProxyList("http", "http");
            
            // Get SOCKS proxies in different formats
            List<String> socks5List = getStaticProxyList("socks5", "socks5");
            List<String> socks5hList = getStaticProxyList("socks5h", "socks5");
            ...
            if (!socks5hList.isEmpty()) {
                System.out.println("SOCKS5H format: " + socks5hList.get(0));
            }
        } catch (Exception e) {
...
Here are examples of what the output might look like for each format:
