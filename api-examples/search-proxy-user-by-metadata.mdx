...
- Find proxy users with a budget within a certain range
...
# Example 2: Search proxy users by metadata range (budget between 1000 and 5000)
# This will find all proxy users that have a budget value between 1000 and 5000
range_url = f"{BASE_URL}/proxy_user/search"
range_params = {
    "min_proxy_user_metadata.monthly_budget": 1000,
    "max_proxy_user_metadata.monthly_budget": 5000
}
...
if range_response.status_code == 200:
    ...
    print(f"\nFound {range_data['total_count']} proxy users with budget between 1000 and 5000:")
    
    # Display the results
    for proxy_user in range_data['data']:
        print(f"  - {proxy_user['proxy_user_id']}: Budget = {proxy_user.get('proxy_user_metadata', {}).get('monthly_budget', 'N/A')}")
else:
...
const rangeSearch = async () => {
  try {
    // Create URL with query parameters
    const url = new URL(`${BASE_URL}/proxy_user/search`);
    url.searchParams.append('min_proxy_user_metadata.monthly_budget', '1000');
    url.searchParams.append('max_proxy_user_metadata.monthly_budget', '5000');
    ...
    // Display the results
    console.log(`\nFound ${data.total_count} proxy users with budget between 1000 and 5000:`);
    data.data.forEach(proxyUser => {
      const budget = proxyUser.proxy_user_metadata?.monthly_budget || 'N/A';
      console.log(`  - ${proxyUser.proxy_user_id}: Budget = ${budget}`);
    });
  } catch (error) {
...
// Example 2: Search proxy users by metadata range (budget between 1000 and 5000)
// This will find all proxy users that have a budget value between 1000 and 5000
$rangeUrl = $baseUrl . '/proxy_user/search?min_proxy_user_metadata.monthly_budget=1000&max_proxy_user_metadata.monthly_budget=5000';
...
if (curl_errno($ch)) {
    ...
    if (isset($rangeData['total_count'])) {
        echo "\nFound {$rangeData['total_count']} proxy users with budget between 1000 and 5000:\n";
        
        // Display the results
        foreach ($rangeData['data'] as $proxyUser) {
            $budget = isset($proxyUser['proxy_user_metadata']['monthly_budget']) ? 
                $proxyUser['proxy_user_metadata']['monthly_budget'] : 'N/A';
            echo "  - {$proxyUser['proxy_user_id']}: Budget = $budget\n";
        }
    } else {
...
func main() {
	...
	params.Add("min_proxy_user_metadata.monthly_budget", "1000")
	params.Add("max_proxy_user_metadata.monthly_budget", "5000")
	...
	// Display the results
	fmt.Printf("\nFound %d proxy users with budget between 1000 and 5000:\n", rangeData.TotalCount)
	for _, proxyUser := range rangeData.Data {
		...
		budget, ok := metadata["monthly_budget"]
		if !ok {
			budget = "N/A"
		}
...
public class SearchProxyUsersByMetadata {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        if (rangeResponse.statusCode() == 200) {
            ...
            System.out.println("\nFound " + totalCount + " proxy users with budget between 1000 and 5000:");
            for (Map<String, Object> proxyUser : proxyUsers) {
                String proxyUserId = (String) proxyUser.get("proxy_user_id");
                ...
                if (proxyUser.containsKey("proxy_user_metadata")) {
                    ...
                    if (metadata.containsKey("monthly_budget")) {
                        budget = metadata.get("monthly_budget");
                    }
                }
                
                System.out.println("  - " + proxyUserId + ": Budget = " + budget);
            }
        } else {
...
- `total_count`: Total number of proxy users matching the search criteria
- `data`: Array of proxy user objects, each containing their metadata
- `page` and `per_page`: Pagination information

## Common Applications

- Group proxy users by department, project, or client
- Find proxy users with budgets or usage limits in specific ranges
- Track custom metadata like deployment dates, owner information, or cost centers