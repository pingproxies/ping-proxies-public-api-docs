...
# Step 1: Get product catalog to find available products
catalog_url = f"{BASE_URL}/user/checkout/catalog"
catalog_response = requests.get(catalog_url, headers=headers)

if catalog_response.status_code != 200:
    print(f"Error retrieving product catalog: {catalog_response.status_code}")
    print(catalog_response.text)
    exit(1)

catalog_data = catalog_response.json()
print("Available products:")
for product_code, product_info in catalog_data["data"].items():
    if product_info["product_type"] == "isp":  # Looking for ISP (static residential) proxies
        print(f"  - {product_code}: {product_info['product_name']}")

# For this example, we'll purchase US static residential proxies (isp_us)
...
if "created" in checkout_data and len(checkout_data["created"]) > 0:
    service_id = checkout_data["created"][0]
    print(f"Service ID: {service_id}")
else:
    print("No service ID found in the response.")
    exit(1)
...
// Step 1: Get product catalog to find available products
axios.get(`${BASE_URL}/user/checkout/catalog`, { headers })
  .then(catalogResponse => {
    console.log('Available products:');
    const catalogData = catalogResponse.data;
    
    // Filter and display ISP (static residential) products
    Object.entries(catalogData.data).forEach(([productCode, productInfo]) => {
      if (productInfo.product_type === 'isp') {
        console.log(`  - ${productCode}: ${productInfo.product_name}`);
      }
    });
  ...
  .then(checkoutResponse => {
    ...
    // Extract service ID from the response
    if (checkoutData.created && checkoutData.created.length > 0) {
      const serviceId = checkoutData.created[0];
...
// Step 1: Get product catalog to find available products
$catalogUrl = $baseUrl . '/user/checkout/catalog';
$ch = curl_init($catalogUrl);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);

$catalogResponse = curl_exec($ch);
$catalogStatusCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
curl_close($ch);

if ($catalogStatusCode != 200) {
    echo "Error retrieving product catalog: " . $catalogStatusCode . "\n";
...
foreach ($catalogData['data'] as $productCode => $productInfo) {
    if ($productInfo['product_type'] === 'isp') {  // Looking for ISP (static residential) proxies
        echo "  - " . $productCode . ": " . $productInfo['product_name'] . "\n";
    }
}
...
// Extract service ID from the response
if (isset($checkoutData['created']) && count($checkoutData['created']) > 0) {
    $serviceId = $checkoutData['created'][0];
    echo "Service ID: " . $serviceId . "\n";
} else {
    echo "No service ID found in the response.\n";
    exit(1);
}
...
func main() {
	...
	// Step 1: Get product catalog to find available products
	catalogURL := fmt.Sprintf("%s/user/checkout/catalog", baseURL)
	catalogReq, err := http.NewRequest("GET", catalogURL, nil)
	if err != nil {
		fmt.Printf("Error creating catalog request: %v\n", err)
		return
	}

	// Add authentication headers
	catalogReq.Header.Add("X-API-Public-Key", apiPublicKey)
	catalogReq.Header.Add("X-API-Private-Key", apiPrivateKey)

	// Execute the catalog request
	...
	if catalogResp.StatusCode != 200 {
		body, _ := ioutil.ReadAll(catalogResp.Body)
		fmt.Printf("Error retrieving product catalog: %d\n", catalogResp.StatusCode)
	...
	if err := json.NewDecoder(catalogResp.Body).Decode(&catalogData); err != nil {
		fmt.Printf("Error parsing catalog response: %v\n", err)
		return
	}

	// Display available ISP (static residential) products
	fmt.Println("Available products:")
	productsData := catalogData["data"].(map[string]interface{})
	for productCode, productInfoRaw := range productsData {
		productInfo := productInfoRaw.(map[string]interface{})
		if productInfo["product_type"] == "isp" {
			fmt.Printf("  - %s: %s\n", productCode, productInfo["product_name"])
		}
	}
	...
	if created, ok := checkoutData["created"].([]interface{}); ok && len(created) > 0 {
		serviceId = created[0].(string)
		fmt.Printf("Service ID: %s\n", serviceId)
	} else {
		fmt.Println("No service ID found in the response.")
		return
	}
...
1. **Catalog Retrieval**: 
   - First, we retrieve the available products from the catalog endpoint
   - This helps us identify the correct product code for the static residential proxies we want to purchase
