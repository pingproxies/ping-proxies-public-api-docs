...
IP address authentication restricts proxy access to specific IP addresses. This is useful for:

- **Enhancing security** by only allowing connections from trusted locations
- **Preventing unauthorized access** even if credentials are compromised
- **Enforcing organizational policies** around proxy usage
...
- Connections must come from a whitelisted IP address
...
The proxy user edit endpoint supports updating several other properties:

### Service Restrictions

```python
# Restrict proxy user to specific services
payload = {
    "proxy_user_is_service_restricted": True,
    "restricted_service_ids": ["srv-12345", "srv-67890"]
}
```
...
```python
# Set bandwidth limits for residential proxies (in bytes)
payload = {
    "proxy_user_residential_bytes_limit": 5368709120  # 5GB
}
```
...
```python
# Add custom metadata for organization and tracking
payload = {
    "proxy_user_metadata": {
...
1. **Audit regularly**: Review and update proxy user authentication settings periodically
2. **Principle of least privilege**: Only grant access to necessary services and IPs
3. **Rotate passwords**: Change passwords regularly for enhanced security
4. **Use metadata**: Leverage metadata for better organization and tracking
5. **Monitor usage**: Keep an eye on proxy usage patterns to detect potential security issues