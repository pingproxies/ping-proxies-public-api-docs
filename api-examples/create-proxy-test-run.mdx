...
description: "Learn how to a list of proxies verify connectivity and performance and then output the results to a csv"
icon: "microscope"
---

This example shows how to test a list of proxies from a text file against a random test server and output the results to a csv.
...
headers = {"X-API-Public-Key": API_PUBLIC_KEY, "X-API-Private-Key": API_PRIVATE_KEY, "Content-Type": "application/json"}
...
def get_available_test_servers():
    ...
    """
    response = requests.get(f"{BASE_URL}/proxy_tester/proxy_test_server/search?proxy_test_server_active=true", headers=headers)
...
def create_bulk_proxy_test(proxy_strings, test_server_id, target_url="https://pingproxies.com"):
    ...
    payload = {"proxies": proxies, "proxy_tester_server_id": [test_server_id], "proxy_test_run_id": test_run_id, "urls": [target_url]}

    # Make request to create the proxy test
    response = requests.post(f"{BASE_URL}/proxy_tester/proxy_test_run/create", json=payload, headers=headers)
...
async function getAvailableTestServers() {
    try {
        const response = await fetch(
            `${BASE_URL}/proxy_tester/proxy_test_server/search?proxy_test_server_active=true`,
            { headers }
        );
...
async function createBulkProxyTest(proxyStrings, testServerId, targetUrl = "https://pingproxies.com") {
    ...
    try {
        const response = await fetch(`${BASE_URL}/proxy_tester/proxy_test_run/create`, {
            method: 'POST',
            headers: headers,
...
class ProxyTester {
    ...
    public function getAvailableTestServers() {
        $url = $this->baseUrl . "/proxy_tester/proxy_test_server/search?proxy_test_server_active=true";
    ...
    public function createBulkProxyTest($proxyStrings, $testServerId, $targetUrl = "https://pingproxies.com") {
        ...
        $url = $this->baseUrl . "/proxy_tester/proxy_test_run/create";
...
func (pt *ProxyTester) GetAvailableTestServers() ([]ProxyTestServer, error) {
	url := BaseURL + "/proxy_tester/proxy_test_server/search?proxy_test_server_active=true"
...
func (pt *ProxyTester) CreateBulkProxyTest(proxyStrings []string, testServerID, targetURL string) (*TestResponse, error) {
	...
	url := BaseURL + "/proxy_tester/proxy_test_run/create"
...
public class ProxyTester {
    ...
    public JsonNode getAvailableTestServers() {
        try {
            String url = baseUrl + "/proxy_tester/proxy_test_server/search?proxy_test_server_active=true";
    ...
    public JsonNode createBulkProxyTest(List<String> proxyStrings, String testServerId, String targetUrl) {
        try {
            ...
            String url = baseUrl + "/proxy_tester/proxy_test_run/create";
...
The example reads proxies from a text file where each line contains a proxy in the format `host:port:username:password`:
```
209.99.134.6:5702:ftrdddgq:518vrc41xyms
...
The script writes comprehensive results to a timestamped CSV file with these columns:
...
Create a `proxies.txt` file in the same directory with your proxy list:
```
# One proxy per line in format: host:port:username:password
209.99.134.6:5702:ftrdddgq:518vrc41xyms
185.199.229.156:7492:another_user:another_pass
45.95.96.132:8691:user3:pass3
```

The script will automatically generate a timestamped CSV file like `proxy_test_results_20250813_143022.csv` with all your test results.
