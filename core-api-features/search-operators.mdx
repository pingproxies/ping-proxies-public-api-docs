...
```
GET /public/user/proxy_user/search?min_proxy_user_creation_datetime=2023-01-01&max_proxy_user_creation_datetime=2023-12-31T23:59:59Z
```
...
Dates and times use 20XX-01-31 format and work with `min_` and `max_` operators. These operators support both date strings and timestamp integers:

```
GET /public/user/service/search?min_service_creation_datetime=2023-06-01
```

```
GET /public/user/service/search?min_service_creation_timestamp=1685577600
```
...
```
GET /public/user/proxy/search?proxy_type=isp&country_id=gb&min_proxy_creation_datetime=2023-01-01&not_proxy_status=pending_deletion
```
