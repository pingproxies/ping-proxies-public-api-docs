...
|-----------|------|-------------|
| `proxy_test_run_id` | string (uuid) | Unique identifier for the test run |
| `proxies` | array | List of proxies to test with their configurations |
| `urls` | array | Target URLs to test against |
| `proxy_tester_server_id` | array | Test servers to use for validation |
...
graph TD
    ...
    D --> E[Collect Results]
    E --> F[Aggregate Performance Data]
    F --> G[Return Test Results]
    G --> H[Optional: Share Results]
```
...
- **Comprehensive Validation**: Each proxy is tested against multiple URLs to ensure consistent functionality
...
- **Batch Size**: Balance between comprehensive testing and API rate limits when testing large proxy pools
