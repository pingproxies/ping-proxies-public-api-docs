---
title: 'Log Summary'
description: 'Understanding the Log Summary object and its role in the Ping Proxies API'
---
...
- **Residential Ledger**: For residential proxies, log summaries relate to bandwidth consumption tracked in the ledger

```mermaid
graph TD
    Customer --> LogSummary
    ProxyUser --> LogSummary
    Logs --> LogSummary
    LogSummary --> ResidentialLedger[Residential Ledger]
```
...
- For residential proxies, the `log_summary_bytes` directly correlates with bandwidth consumption
...
- Log Summaries complement the Residential Ledger but serve a different purpose:
  - Log Summaries focus on usage patterns across all proxy types
