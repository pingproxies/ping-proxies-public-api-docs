...
Our two UDP servers, running on `demo.residential.pingproxies.com` also act as an echo server and an IP address reporting service.
...
On our services, we support Prior Knowledge and ALPN. The examples below show how to use both methods.
	...
	<Tabs>
		<Tab title="Python">
			...
			By re-using a `httpx.Client`, the underlying TCP connection to the proxy server is kept for each client. As such, sending three requests requires only 4 TCP connections to be made. If this was done using HTTP/1.1, 6 TCP connections would need to be made: three to the proxy and three to the targets.
		</Tab>
		...
		<Tab title="Rust">
			...
			async fn main() {
			    ...
			    let request = async move {
			        ...
			        println!("REQUEST:\r{}", String::from_utf8_lossy(&bytes[..]));
			...
			HTTP/3 is still relatively new and unsuppoted. HTTP/3 proxying even more so.
		</Tab>
	</Tabs>
	</Accordion>
